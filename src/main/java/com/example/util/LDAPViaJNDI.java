...
@Component("ldapViaJNDI")
@Transactional
@Configuration
@PropertySource("classpath:config/ldap.properties")
public class LDAPViaJNDI {
	...
	public UserProfile isValidUser(String userName) {
		
		String ldapURL=environment.getProperty("ldap.url");
		String userEmail=environment.getProperty("ldap.principal");
		...
		try {
			...
			while (results.hasMore()) {
				...
				user.setUserName(attributes.get("displayName").get().toString());
				user.setManagerName(name[1]);
